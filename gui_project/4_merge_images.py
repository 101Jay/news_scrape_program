...
def merge_image(): 
    ...
    widths, heights = zip(*(i.size for i in images)) # zip의 활용

    # 가로는 최댓값, 높이는 모두 합친 값을 사용해야 모든 이미지들을 더할 수 있다(세로로 더함)
    ...
    save_path = os.path.join(txt_save_path.get(), "merge_phot.jpg")
